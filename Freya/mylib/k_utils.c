...
static void kproc_foreach(kptr_t proc, bool (^match)(kptr_t, pid_t))
{
    ...
    while (KERN_POINTER_VALID(proc)) {
        pid = kapi_read32(proc + OFFSET(proc, p_pidPF));
        if (g_exp.debug) {
            util_info("pid %u", pid);
            util_msleep(100);
        }
        ...
        next = kapi_read_kptr(proc + OFFSET(proc, le_nextPF));
        if (next == KPTR_NULL) {
            break;
        }
        proc = next;
...
static void kproc_foreach_reverse(kptr_t proc, bool (^match)(kptr_t, pid_t))
{
    ...
    while (KERN_POINTER_VALID(proc)) {
        ...
        prev = kapi_read_kptr(proc + OFFSET(proc, le_prevPF));
        if (prev == KPTR_NULL) {
            break;
        }
        proc = prev - OFFSET(proc, le_nextPF);
    }
}
...
kptr_t ipc_entry_lookup(mach_port_t port_name)
{
    ...
    uint32_t table_size = kapi_read32(itk_space + OFFSET(ipc_space, is_table_sizePF));
    uint32_t port_index = MACH_PORT_INDEX(port_name);
    if (port_index >= table_size) {
        util_warning("invalid port name %#x", port_name);
        return 0;
    }
    kptr_t is_table = kapi_read_kptr(itk_space + OFFSET(ipc_space, is_tablePF));
    kptr_t entry = is_table + port_index * SIZE(ipc_entryPF);
    return entry;
}
...
kptr_t port_name_to_ipc_port(mach_port_t port_name)
{
    ...
    kptr_t ipc_port = kapi_read_kptr(entry + OFFSET(ipc_entryPF, ie_objectPF));
    return ipc_port;
}
...
kptr_t port_name_to_kobject(mach_port_t port_name)
{
    ...
    kptr_t kobject = kapi_read_kptr(ipc_port + OFFSET(ipc_port, ip_kobjectPF));
    return kobject;
}
...
void debug_dump_ipc_port(mach_port_t port_name, kptr_t *kobj)
{
    ...
    kptr_t object = kapi_read_kptr(entry + OFFSET(ipc_entryPF, ie_objectPF));
    uint32_t ip_bits = kapi_read32(object + OFFSET(ipc_port, ip_bitsPF));
    uint32_t ip_refs = kapi_read32(object + OFFSET(ipc_port, ip_referencesPF));
    kptr_t kobject = kapi_read_kptr(object + OFFSET(ipc_port, ip_kobjectPF));
...
{
    kptr_t proc_p_ucred = kapi_read_kptr(proc + OFFSET(proc, p_ucredPF));
    kptr_t p_ucred_cr_label = proc_p_ucred + OFFSET(ucred, cr_posixPF);

    char old_cred[SIZE(posix_credPF)];
    kapi_read(p_ucred_cr_label, old_cred, SIZE(posix_credPF));

    kptr_t cr_label = kapi_read_kptr(p_ucred_cr_label + SIZE(posix_credPF));
...
struct kOSDict *kernel_fetch_dict(kptr_t dict_addr)
{
    ...
    uint32_t cap = *(uint32_t *)(obj + OFFSET(OSDictionary, capacityPF));
    struct kOSDict *dict;
    ...
    dict->cap = cap;
    dict->count = *(uint32_t *)(obj + OFFSET(OSDictionary, countPF));
    dict->items_addr = kapi_read_kptr(dict_addr + OFFSET(OSDictionary, dictionaryPF));
    char *ptr = dict->data;
    ...
    for (int i = 0; i < dict->count; i++) {
        ...
        // PACed in iOS 14.3
        kptr_t string = *(kptr_t *)(obj + OFFSET(OSString, stringPF));
...
{
    kptr_t proc_p_ucred = kapi_read_kptr(proc + OFFSET(proc, p_ucredPF));
    kptr_t p_ucred_cr_label = proc_p_ucred + OFFSET(ucred, cr_posixPF) + SIZE(posix_credPF);
...
void proc_write_MACF(kptr_t proc, struct kOSDict *macf)
{
    ...
    kapi_write32(macf->self_addr + OFFSET(OSDictionary, countPF), macf->count);
